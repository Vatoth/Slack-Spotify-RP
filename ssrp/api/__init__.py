...
class API:
    ...
    def check_token(self):
        if not self.test_spotify_token():
            if not self.test_spotify_token():
                error(
                    "Couldn't re-authenticate with Spotify! Please delete your config and go through setup again!"
                )
                exit()
            else:
                return True
    ...
    def refresh_access_token(self):
        res = requests.get(
            "https://ssrp.maxbridgland.com/getRefreshForSpotify/" + self.refresh_token
        )
        if res.status_code == 200:
            resobj = res.json()
